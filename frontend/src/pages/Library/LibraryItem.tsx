...
const LibraryItem = ({
    ...
    // 直播内容(isDirectPlay)使用上游的进度逻辑
    const progress = isFolder
        ? folderProgress
        : isDirectPlay
          ? (item.UserData?.Played && watched <= 0 ? 100 : (runtime > 0 ? (watched / runtime) * 100 : 0))
                ...
                {shouldRenderFolderIcon ? (
                    ...
                    <>
                        <img
                            ...
                            src={finalPosterUrl}
                            alt={item.Name || t('library:no_title')}
                            style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: posterFit, objectPosition: 'center', zIndex: 10 }}
