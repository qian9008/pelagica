...
const ItemDisplay = ({ item, overlay, posterAspectRatio = '2/3' }: ItemDisplayProps) => {
    const { t } = useTranslation('item');
    const [posterError, setPosterError] = useState(false);

    return (
        <Link to={`/item/${item.Id}`} key={item.Id} className="p-0 m-0">
            <div className={`relative w-full aspect-${posterAspectRatio} overflow-hidden rounded-md group`}>
                {!posterError ? (
                    <>
                        <img
                            key={item.Id}
